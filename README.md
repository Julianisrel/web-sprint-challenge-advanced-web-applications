...
3. Describe how web servers work.
  -- Web servers or backend computer  serve files(webpages,images,etc) to clients(the user). when a user enters a url into their browser, it then fetches the website from the server and if it finds it, it processes all requests made by the user for example if they requested a search and then would send it back to display it in the users browser
